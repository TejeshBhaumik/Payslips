...
- A popup saying `hit limit, please retry in five minutes` when the send limit is reached.

## Rate Limit Behavior

`sendEmail.reserve_send_slot()` raises `RateLimitExceeded` when the batch has hit `MAX_EMAILS_PER_BATCH`.
